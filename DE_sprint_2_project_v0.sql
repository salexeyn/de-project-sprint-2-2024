...
SELECT  order_id,
        ...
        customer_birthday,
        customer_email 
FROM external_source.craft_products_orders as base
  left join external_source.customers as cust on base.customer_id = cust.customer_id
...
CREATE TABLE dwh.customer_report_datamart (
	...
	top_craftsman_id int8 NOT NULL, --- идентификатор самого популярного мастера ручной работы у заказчика. Если заказчик сделал одинаковое количество заказов у нескольких мастеров, возьмите любого;
    ...
    SELECT  
            ...
            FROM (
                SELECT     -- в этой выборке объединяем две внутренние выборки по расчёту столбцов витрины и применяем оконную функцию для определения самой популярной категории товаров
                        ...
                        t3.top_craftsman_id,
                        t5.product_type,
                        RANK() OVER(PARTITION BY T5.customer_id_for_product_type ORDER BY count_product DESC) AS rank_count_product ,
                        row_number() OVER(PARTITION BY T2.customer_id ORDER BY count_craftsman_id DESC) AS rank_count_craftsman_id 
                        FROM ( 
                                ...
                                INNER JOIN (
                                    SELECT     -- Эта выборка поможет определить самый популярный товар у мастера ручной работы. Эта выборка не делается в предыдущем запросе, так как нужна другая группировка. Для данных этой выборки можно применить оконную функцию, которая и покажет самую популярную категорию товаров у мастера
                                            ...
                                            FROM dwh_delta AS dd
                                                GROUP BY dd.craftsman_id, dd.customer_id
                                                    ORDER BY count_craftsman_id DESC) AS T3 ON T2.customer_id = T3.customer_id
                ...
                ) AS T4 WHERE T4.rank_count_craftsman_id = 1 and rank_count_product = 1
...
dwh_delta_update_result AS ( -- делаем перерасчёт для существующих записей витринs, так как данные обновились за отчётные периоды. Логика похожа на insert, но нужно достать конкретные данные из DWH
    SELECT  
            ...
            FROM (
                ...
                ) AS T4 WHERE T4.rank_count_craftsman_id = 1 and rank_count_product = 1 ORDER BY report_period -- условие помогает оставить в выборке первую по популярности категорию товаров
